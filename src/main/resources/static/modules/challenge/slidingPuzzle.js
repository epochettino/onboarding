    ...
    return {
        ...
        link: function (scope, element, attributes) {
            element.bind("change", function (changeEvent) {
                scope.fileread = new FileReader();
                scope.$apply(function () {
                        scope.fileread = "img/" + changeEvent.target.files[0].name;
                    });
            });
        }
    }
...
module.directive('fdInput', [function () {
    return {
        scope: {
            fdInput: "="
        },
        link: function (scope, element, attrs) {
            element.on('change', function  (evt) {
                var files = evt.target.files[0].name;
                scope.fdInput = files[0].name;
            });
        }
    }
}]);
